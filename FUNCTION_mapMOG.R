...
mapMOG <- function(locale, source.path, interpolate = TRUE, recent = TRUE, API = "rFIA", resolution = 1590, power = 2){ #begin function
  ...
  if(API %in% c("rfia","rFIA","RFIA","Rfia","RFia")){ #begin instructions for if rFIA package is being used (allowing for different spellings)
    ...
    if(locale.state$STUSPS %in% c("PR","AK","HI","VI","GU")){stop("Functionality for locales outside of the Contiguous United States is not supported")}
    ...
    if(locale.state$STUSPS %in% c("CA","NV")){#begin FIA data fetch for Region 5, which needs the sitetree table
      ...
      nwfp <- st_read(paste(source.path,"/utility_NWFPboundary.geojson", sep="")) #load boundary for northwest forest plan
      nwfp <- st_as_sf(st_union(nwfp)) #collapse northwest forest plan into a single shapefile
            ...
            if(nrow(mat.df) > 0){#begin calculating indices/metrics if data are available
              ...
              if(sum(is.na(c(snagbatot,HTsd,HTquart,ddiscore,QMDdom,badom,tpadom))) == 0){#begin processing if all tree metrics are available
                if(region %in% "eastern"){#begin instructions for eastern region 
                  ...
                  #conditions[c,"MOG"] <- max(MOG.vector, na.rm=TRUE) #save MOG status (0 = young, 0.5 = mature, 1 = old growth)
                ...
                if(region %in% "southern"){#begin instructions for southern region
                  ...
                  if(length(southern.type > 0)){#begin instructions if condition fits within a defined southern forest type
                    ...
                    for(s in 1:length(southern.type)){#begin looping through southern forest types (s)
                      ...
                      #southern.MOG.status <- c(southern.MOG.status, local.MOG.status)
  ...
  if(interpolate == TRUE){ #begin interpolation process if specified by used (default = TRUE)
    ...
    #load/install necessary packages
    #if (!require(FedData)){ message("Installing required package 'FedData.'")}
    #if (!require(FedData)){ devtools::install_github("ropensci/FedData")} #Does user have the package downloaded? If so move on, if not, download.
    #library(FedData) #load library
    
    #print progress message
    message("Accessing landcover data for interpolation (interpolate = TRUE)")
    
    #pull in NLCD
    #NLCD.small <- get_nlcd(template = locale, #clip raster to study area
    #                       label = "canopy", #Label for raster (not sure why - can probably skip this)
    #                       year = 2019, #specify data year
    #                       dataset = "canopy", #specify which dataset you want
    #                       force.redo = TRUE) #don't rely on the data previously downloaded (likely applies to an alternative locale)
    
    NLCD.small <- terra::rast(paste(source.path,"/utility_canopy/nlcd_tcc_conus_2021_v2021-4.tif",sep="")) #load canopy data
    ...
    #if(minmax(NLCD)[2] < canopy.threshold){stop("Study area is insufficiently forested (< 1 square mile of contiguous forest). Imputation cancelled.")} #print error message if too little forest in study area
    
    
    #develop set of rules for reclassifying the raster to be forest or an obviously fake value
    #reclass.rules <- as.data.frame(matrix(c(canopy.threshold,100))) #save only cells with at least 50% forest cover but no more than 100% (values over water recieve erroneous values greater than 100)
    ...
    #calculate distance between each gridcell and nearest 3 FIA plots
    #NLCD.trees.vect <- terra::as.points(NLCD.trees) #convert tree raster to points
    #temp.locale <- terra::vect(locale) #temporarily convert locale to terra format
    # temp.locale <- terra::project(x = temp.locale, #reproject locale
